...
int main()
{
	...
	for (int i = 0; i < 816; i++) {
		...
		int currentcolumn = 0;
		if (input[i][0] == 'B') {
			currentrow = currentrow + 64;
		}
		if (input[i][1] == 'B') {
			currentrow = currentrow + 32;
		}
		if (input[i][2] == 'B') {
			currentrow = currentrow + 16;
		}
		if (input[i][3] == 'B') {
		...
		if ((currentrow * 8) + currentcolumn > highest) {
			highest = (currentrow * 8) + currentcolumn;
		}
		ID[i] = currentrow * 8 + currentcolumn;
	}
	cout << "Highest is " << highest << "\n \n";


	for (int i = 0; i < highest; i++) { // highest = 848, ID's = 816, difference 32, coincidence? Tip: "Your seat wasn't at the very front or back".
		bool found = false;
		for (int k = 0; k < 816; k++) {
			if (ID[k] == i) {
				found = true;
		...
		if (!found) {
			cout << i << " was not found in the ID list \n";
		}
	}
}
